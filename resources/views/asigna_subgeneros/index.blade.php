...
@section('dashboard-content')
    <div class="container py-4">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h3 class="mb-0 text-dark fw-bold">Asignación de Subgéneros</h3>
            <button type="button" class="btn btn-link p-0 text-decoration-none fs-2"
                    data-bs-toggle="modal" data-bs-target="#modalCreateAsignaSubgenero"
            ...
            <table class="table table-bordered table-striped mi-datatable" style="width:100%">
                ...
                @foreach($asignaciones as $asignacion)
                    <tr>
                        ...
                        <td class="fw-semibold">{{ $asignacion->titulo_libro }}</td>
                        <td>{{ $asignacion->nombre_subgenero }}</td>
                        <td class="text-end">
                            <button type="button" class="btn btn-link p-0 text-decoration-none fs-5 me-3"
                                    data-bs-toggle="modal" data-bs-target="#modalEditAsignaSubgenero{{ $asignacion->id }}"
                            ...
                            <button type="button" class="btn btn-link p-0 text-decoration-none fs-5"
                                    data-bs-toggle="modal" data-bs-target="#modalDeleteAsignaSubgenero{{ $asignacion->id }}"
    ...
    <div class="modal fade" id="modalCreateAsignaSubgenero" tabindex="-1" aria-hidden="true">
            ...
            <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
                <div class="modal-header border-0" style="background-color: #4b1c71; color: white; border-radius: 20px 20px 0 0;">
                    <h5 class="modal-title bebas fs-4">
                        <i class="fa-solid fa-circle-plus me-2"></i> Nueva Asignación
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <form action="{{ route('asigna_subgenero.store') }}" method="post">
                    @csrf
                    <div class="modal-body p-4">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-bold" style="color: #4b1c71;">Subgénero</label>
    ...
    @foreach($asignaciones as $asignacion)
        <div class="modal fade" id="modalEditAsignaSubgenero{{ $asignacion->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
                    <div class="modal-header border-0" style="background-color: #4b1c71; color: white; border-radius: 20px 20px 0 0;">
                        <h5 class="modal-title bebas fs-4">
                            <i class="fa-solid fa-pen-to-square me-2"></i> Editar Asignación
                        </h5>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>

                    <form action="{{ route('asigna_subgenero.update', $asignacion->id) }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="modal-body p-4">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    ...
                                    <select name="libro_id" class="form-select" required>
                                        <option value="">Seleccione</option>
                                        @foreach($librosCatalogo as $libro)
                                            <option value="{{ $libro->id }}" {{ $asignacion->libro_id == $libro->id ? 'selected' : '' }}>
                                                {{ $libro->titulo }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select name="subgenero_id" class="form-select" required>
                                        <option value="">Seleccione</option>
                                        @foreach($subgenerosCatalogo as $subgenero)
                                            <option value="{{ $subgenero->id }}" {{ $asignacion->subgenero_id == $subgenero->id ? 'selected' : '' }}>
                                                {{ $subgenero->nombre }}
                                            </option>
                                        @endforeach
                                    </select>
        ...
        <div class="modal fade" id="modalDeleteAsignaSubgenero{{ $asignacion->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content border-0 shadow-lg" style="border-radius: 20px;">
                    ...
                    <div class="modal-body p-4 text-center">
                        <p class="fs-5 mb-1">¿Estás seguro de eliminar esta asignación?</p>
                    ...
                    <div class="modal-footer border-0 p-4 pt-0 justify-content-center">
                        ...
                        <form action="{{ route('asigna_subgenero.destroy', $asignacion->id) }}" method="post" class="d-inline">
                            @csrf
                            @method('DELETE')
